...
class SettingsGroupField extends SettingsGroup
{
	...
	public function render_textarea($field)
	{
		...
		if (!empty($this->setting_group_name)) {
			$field['name'] = sprintf("{$this->setting_group_name}[%s]", $field['id']);
			$value         = $this->get_option($field['id'], $default_value);
		} else {
			$field['name'] = $field['id'];
		...
		$attributes = wp_parse_args($field, $defaults);
		$textarea   = Html::tag('textarea', $value, $attributes);
	...
	public function render_checkbox($field)
	{
		...
		unset($field['options'], $field['input_class'], $field['multiple']);
		...
		if ($is_multiple) {
			echo Form::hidden([
				'name'  => sprintf("{$this->setting_group_name}[%s][]", $field['id']),
			]);
		}
